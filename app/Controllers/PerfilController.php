...
class PerfilController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['user'] = $this->userModel->find($usuarioId);

        if (!$data['user']) {
            log_message('error', 'PerfilController::index() - Usuario ID ' . $usuarioId . ' no encontrado en la base de datos.');
            $session->destroy(); // Limpiar sesión si el usuario no existe
            return redirect()->to('/login')->with('error', 'Usuario no encontrado. Inicie sesión nuevamente.');
        }

        // Obtener dispositivos enlazados por el usuario
        $data['dispositivosEnlazados'] = $this->dispositivoModel
                                            ->where('id_usuario', $usuarioId) E
                                            ->findAll();

        log_message('debug', 'PerfilController::index() - Dispositivos encontrados: ' . count($data['dispositivosEnlazados']));
        
        // Carga la vista principal del perfil
    ...
    public function enviarVerificacion()
    {
        ...
        // 1. Generar Token de acceso temporal y actualizar en la DB
        ...
        // 3. Configurar el email (usa la clase Email personalizada con SendGrid)
        $email = new Email();
        $datosEmail = [
            'email' => $user['email'],
            'asunto' => 'Verificación de Acceso a Configuración de Perfil ASG',
            // Asegúrate de que tienes la vista APPPATH/Views/emails/verificacion_perfil.php
    ...
    public function confirmarAcceso($token = null)
    {
        ...
        // Establecer una bandera de sesión para dar acceso temporal a la configuración (10 minutos)
