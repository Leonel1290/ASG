...
{
    try {
        // DECODIFICAR la MAC (los %3A se convierten de vuelta a :)
        $mac_decoded = urldecode($mac);
        
        log_message('debug', 'MAC recibida: ' . $mac);
        log_message('debug', 'MAC decodificada: ' . $mac_decoded);
        
        // Limpiar y validar la MAC
        $mac_clean = strtoupper(str_replace(['%3A', '%3a'], ':', $mac_decoded));
        
        // Validar formato de MAC (acepta con : o -)
        if (!preg_match('/^([0-9A-F]{2}[:-]){5}([0-9A-F]{2})$/', $mac_clean)) {
            log_message('error', 'Formato de MAC inválido: ' . $mac_clean);
            return $this->response->setStatusCode(400)->setJSON([
                'status' => 'error',
                'message' => 'Formato de MAC inválido: ' . $mac_clean
            ]);
        }
        ...
        if ($lectura) {
            ...
            $dispositivo = $this->dispositivoModel->where('MAC', $mac_clean)->first();
            $estado_valvula = $dispositivo ? (bool)$dispositivo['estado_valvula'] : false;
            ...
            return $this->response->setJSON([
                ...
                'mac' => $mac_clean // Para debugging
            ]);
        } else {
            log_message('info', 'No se encontraron lecturas para MAC: ' . $mac_clean);
            return $this->response->setJSON([
                'status' => 'error',
                'message' => 'No se encontró lectura para este dispositivo: ' . $mac_clean
            ]);
        }
